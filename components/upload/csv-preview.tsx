      ...
      <div className="flex items-center justify-between gap-4 p-4 bg-slate-50 rounded-xl border border-slate-200">
        ...
        <div className="flex items-center gap-6 text-sm">
          <div>
            ...
            <span className="font-semibold text-slate-800">{formatINR(totalGross)}</span>
          </div>
          <div>
            <span className="text-slate-500">Total Net:</span>{" "}
            <span className="font-semibold text-emerald-700">{formatINR(totalNet)}</span>
          </div>
          {errorCount > 0 && (
          ...
          <tbody>
            {rows.map((row, i) => (
              <tr key={i}>
                ...
                <td className="text-right text-slate-600">{formatINR(row.hra)}</td>
                <td className="text-right text-slate-600">{formatINR(row.allowances)}</td>
